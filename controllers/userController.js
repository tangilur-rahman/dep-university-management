...
// for get all administrators when admin
const getAllAdministrators = async (req, res) => {
	try {
		const adminsDoc = await adminModel.find({});

		res.status(200).json(adminsDoc);
	} catch (error) {
		res.status(500).json({ error: "Something was wrong, Try again!" });
	}
};
	...
	try {
		...
		if (student) {
			...
			if (advisor) {
				...
				const administrator = await adminModel.findOne({ _id: req.params._id });

				if (administrator) {
					res.status(200).json(administrator);
				} else {
					req.status(404).json({ error: "Didn't find that user." });
				}
			}
		}
	} catch (error) {
...
// for update administrator by admin
const updateAdministrator = async (req, res) => {
	try {
		const { _id, getName, getId, getEmail, getGender, newPassword } =
			req.body.userDocument;

		if (getName) {
			await adminModel.updateOne({ _id }, { $set: { name: getName } });
		}

		if (getId) {
			const document = await adminModel.findOne({ id: getId });

			if (document) {
				throw Error("That Id already used!");
			} else {
				await adminModel.updateOne({ _id }, { $set: { id: getId } });
			}
		}

		if (getEmail) {
			const documentAdmin = await adminModel.findOne({ email: getEmail });
			const documentAdvisor = await advisorModel.findOne({ email: getEmail });
			const documentStudent = await studentModel.findOne({ email: getEmail });

			if (documentAdmin || documentAdvisor || documentStudent) {
				throw Error("That email already used!");
			} else {
				await adminModel.updateOne({ _id }, { $set: { email: getEmail } });
			}
		}

		if (getGender) {
			await adminModel.updateOne({ _id }, { $set: { gender: getGender } });
		}
...
// for search administrator by admin
const searchAdministrator = async (req, res) => {
	try {
		const getAdministrator = await adminModel.find({
			$or: [
				{
					id: {
		...
		res.status(200).json(getAdministrator);
	} catch (error) {
		res.status(500).json({ error: "Not Found Any Administrator" });
	}
};
