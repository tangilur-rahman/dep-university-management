		...
		if (!cpassword) {
			...
			if (comparePassword) {
				...
				if (newPassword && getPhone && active_status) {
					...
					// when only admin name
					if (getName || req.currentUser.role === "administrator") {
						await adminModel.updateOne(
							{ _id: req.currentUser._id },
							{
								$set: { name: getName }
					...
					// when only admin email
					if (getEmail || req.currentUser.role === "administrator") {
						const documentAdmin = await adminModel.findOne({ email: getEmail });
						const documentAdvisor = await advisorModel.findOne({
							email: getEmail
						});
						const documentStudent = await studentModel.findOne({
							email: getEmail
						});
					...
					// when only admin id
					if (getId || req.currentUser.role === "administrator") {
						const documentAdmin = await adminModel.findOne({ id: getId });
						const documentAdvisor = await advisorModel.findOne({
							id: getId
						});
						const documentStudent = await studentModel.findOne({
					...
					if ((getName || getEmail || getId) && !newPassword) {
						res
							.status(200)
							.json({ message: "Administrator info updated successfully." });
					} else if (getPhone) {
						res
							.status(200)
